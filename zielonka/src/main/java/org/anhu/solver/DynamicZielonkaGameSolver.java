 ...
 *
 */
public class DynamicZielonkaGameSolver extends BasicZielonkaGameSolver
{
    ...
    private GameResult solve(ParityGame game, DynamicSCCDecomposition decomposition, DynamicSCCdecomposer decomposer){
        ...
        while(!decomposition.isEmpty()){ // then there is a final SCC
            ...
            SCCTreeNode finalSCC = decomposition.getFinalSCCTree();
            ...
            GameResult finalIntermediateResult = intermediateResult;
            List<Integer> attractorsetPlayer0 = findAttractorset(game, gameStateG, finalIntermediateResult.getVerticesWonByPlayer0(), PLAYER0);
            List<Integer> attractorsetPlayer1 = findAttractorset(game, gameStateG, finalIntermediateResult.getVerticesWonByPlayer1(), PLAYER1);
            ...
            for(int member: sccMembers){
                verticesToBeRemoved.remove(member);
            }

            // the final scc should have been fully solved
            Statistic.registerExecutionTime(Statistic.Event.UPDATEDYNAMICDECOMP, () -> decomposition.removeTree(game, finalSCC));
