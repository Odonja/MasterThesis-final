 ...
 *
 */
public class StaticZielonkaGameSolver2 extends BasicZielonkaGameSolver
{
    ...
    private GameResult solve(ParityGame game, GameState gameStateG, StaticSCCdecomposer decomposer){
        ...
        while(!sccDecomposition.SCCs.isEmpty()){
            List<SCC> finalSCCs = findAllFinalSCCs(gameStateG.verticesInGame, sccDecomposition.SCCs);
            GameResult iterationResult = new GameResult();
            for(SCC finalSCC : finalSCCs){
             ...
             List<Integer> attractorsetPlayer0 = findAttractorset(game, gameStateG.verticesInGame, iterationResult.getVerticesWonByPlayer0(), PLAYER0);
             List<Integer> attractorsetPlayer1 = findAttractorset(game, gameStateG.verticesInGame, iterationResult.getVerticesWonByPlayer1(), PLAYER1);
    ...
    private List<SCC> findAllFinalSCCs(boolean[] inGameVertices, List<SCC> sccs){
        List<SCC> finalSCCs = new ArrayList<>();
        for(SCC scc : sccs){
            boolean isFinalSCC = true;
            for (int outgoingEdge : scc.outgoingEdges) {
                if(inGameVertices[outgoingEdge]){
